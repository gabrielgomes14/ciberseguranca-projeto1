...
def render_status_metrics(resumos: dict[str, ResultadoTema], label_itens: str = "itens") -> None:
    ...
    parciais = sum(r.parciais for r in resumos.values())
    nao_conformes = sum(r.nao_conformes for r in resumos.values())
    na = sum(r.na for r in resumos.values())
    nao_avaliado = total - (conformes + parciais + nao_conformes + na)
    base_calc = conformes + parciais + nao_conformes

    col1, col2, col3, col4, col5 = st.columns(5)
    col1.metric("✅ Conforme", conformes)
    col2.metric("🟡 Parcial", parciais)
    ...
    st.caption(
        ...
        f"(Conformes + Parciais + Não Conformes). "
