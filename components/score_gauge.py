...
# Cor de destaque (azul) usada em traços e preenchimentos. Funciona bem em ambos
# os temas porque os elementos textuais usam a cor padrão herdada do Streamlit.
_PRIMARY = "#3b82f6"
...
def render_radar(temas: list[str], scores: list[float], *, key: str | None = None) -> None:
    ...
    fig = go.Figure(
        go.Scatterpolar(
            ...
            fill="toself",
            line={"color": _PRIMARY},
            fillcolor=_hex_to_rgba(_PRIMARY, 0.25),
            name="Score",
        )
    )
    fig.update_layout(
